...
route.put('/:id', async (req, resp) => {
    ...
    try {
        ...
        if (!donacionActualizada) {
            return resp.status(404).json({ mesaje: "Donación no encontrada" });
        }
...
route.delete('/:id', async (req, resp) => {
    ...
    try {
        const donacionEliminada = await Donacion.findByIdAndDelete(
            req.params.id
        );
        ...
        resp.status(200).json({ mesaje: 'Donación eliminada' });
    } catch (error) {
        resp.status(400).json({ mesaje: error.message });
    }
